...
fn main() {
    ...
    let (stream, _) = loop {
        match listener.accept() {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                assert!(std::time::Instant::now() < deadline, "Firecracker connect timed out");
    ...
    let faults = std::cell::Cell::new(0_u64);
            ...
            for event in events_to_handle.drain(..) {
                ...
                match event {
                    userfaultfd::Event::Pagefault { addr, .. } => {
                        ...
                        if faults.get().is_multiple_of(1024) {
                            eprintln!("served_page_fault_events={}", faults.get());
                        }
