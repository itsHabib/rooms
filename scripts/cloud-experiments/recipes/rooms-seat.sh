...
up() {
    ...
    (cd "$HOME" && setsid nohup python3 -m http.server "$BIN_PORT" --bind "$HOST_IP" < /dev/null > "$STATE/http.log" 2>&1 & echo $! > "$STATE/http.pid")
...
seat_index() { # allocate a clone to a seat name, once
    local f="$STATE/seats/$SEAT"
    if [ ! -f "$f" ]; then
        local used n
        used=$(ls "$STATE/seats" | wc -l)
        n=$(cat "$STATE/count")
        [ "$used" -lt "$n" ] || { echo "no free clone for seat $SEAT" >&2; exit 1; }
        echo $((used + 1)) > "$f"
    fi
    cat "$f"
}
...
turn() {
    ...
    # The seat reads /run/rooms/secrets.env itself; nothing here exports the token.
    guest "$idx" "export PATH=/nix/var/rooms/env/bin:\$PATH SWARM_STORE='$SWARM_STORE' SWARM_INCARNATION=\$(cat /proc/sys/kernel/random/uuid); cd ~ && ~/swarm seat run --seat '$SEAT' --prompt ~/prompts/$SEAT.txt --dir ~/work/$SEAT --remote '$remote' --resume '${RESUME:-}' --model '${MODEL:-}' --turns '${TURNS:-0}' --skip-permissions"
}
