...
GUEST
'''
assert shutil.which("fincore"), "fincore required before starting VMs"
rows=[]
for n in [16,32,64]:
    name=f'shared-store-v2-{n}'
    samples=[]
    sampling_errors=[]
    ...
    def sample():
        try: collect_samples()
        except Exception as e: sampling_errors.append(repr(e))
    ...
    row={'sampling_errors':sampling_errors,'sample_count':len(samples),'n':n,'elapsed_seconds':rec['elapsed_seconds'],'cli_exit':rec['cli_exit'],'receipt_count':len(receipts),'unique_store_digests':sorted(set(x['digest'] for x in receipts)),'host_after':r.host_state()}
    rows.append(row);(r.LAB/'shared-store-v2-results.json').write_text(json.dumps(rows,indent=2))
    print('SHARED_STORE',n,rec['cli_exit'],len(receipts),flush=True)
    if sampling_errors or not samples: raise RuntimeError(f'Invalid memory measurement: {sampling_errors}')
