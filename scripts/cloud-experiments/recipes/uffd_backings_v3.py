...
r.BIN=r.READY/'lab/rooms-uffd-v3'
rows=[]
...
for trial,order in enumerate([list(variants),list(reversed(variants)),['tmpfs-never','disk','tmpfs-always']]):
 for kind in order:
  ...
  rows.append(row);(r.LAB/'uffd-v3-results.json').write_text(json.dumps(rows,indent=2));print('BACKING',kind,trial,rec['cli_exit'],rec['elapsed_seconds'],flush=True)
