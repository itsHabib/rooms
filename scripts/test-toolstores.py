...
def run(args, name, command, expected=0, store=None, extra=(), memory=1024):
    ...
    assert result['command'] == ['sh', '-c', command], (name, result['command'])
    started = next(event for event in history if event['event'] == 'workload_started')
    assert started['command'] == result['command'], (name, started)
