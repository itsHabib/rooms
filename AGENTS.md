...
**The Claude trigger comment must be `@claude review` and nothing else.** [`.github/workflows/claude.yml`](.github/workflows/claude.yml) checks out the PR head, runs the reviewer, and then attests that head to gate as a `<!-- gate:review-attestation -->` comment — but only when the *whole* comment body is a bare review request (`@claude review`, optionally `please`, optionally trailed by `this` / `this PR` / `the PR` and a `.` or `!`). Add a scope (`@claude review the failing CI logs`) or extra instructions on a later line and the review still happens while the attestation does not, so gate parks the PR on `review panel incomplete: missing=[claude]`. The match is narrow on purpose: `@claude the code-review tool is broken` must never clear a panel.

The attestation also requires published evidence that the review actually happened — a formal review or a fully-checked task list from `claude[bot]` inside the run's own window — and that the PR head has not moved since checkout. The action exiting 0 is not proof; a stub reply exits 0 too.
