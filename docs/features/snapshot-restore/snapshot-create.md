...
- Mocked failure tests cover output rejection under both room and jail trees, writable jail-target staging/ownership/symlink refusal, pause, create, jail-to-host collection and partial copies, missing/empty files, artifact sync, recoverable versus terminal partial-create classification, exact-claim abort, slot-token file/directory sync, GC before reservation, reservation-before-termination, concurrent GC at the handoff, incomplete teardown, metadata publication, indexed discovery with the base absent, and transaction recovery at every boundary.
