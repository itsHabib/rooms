...
- Before snapshot creation, atomically create a per-base exclusive pending intent at `<state-base>/snapshot-intents/<base-room-id>.json`; `create_new` plus file and directory sync must reject a second transaction for the same base before either invocation pauses the VM. The intent embeds the snapshot id, complete planned `SnapshotMeta` verbatim, artifact paths, and completed transaction boundaries. Capture `created_at`, Firecracker version, rootfs hash, repo SHA, provenance, base identity, slot, and guest IP before the first effect so base-absent recovery publishes the original descriptor without recomputing mutable or unavailable facts. `snapshot-recover <snapshot-id>` finds the transaction by scanning the indexed values. This stable index is outside the room and output trees and remains discoverable after either is absent.
...
- Make `rooms kill <base-id>` consult the same per-base intent before signaling or ordinary reap. While an exact snapshot transaction is pending, reject kill with the transaction/snapshot id and direct the caller to `snapshot-recover`; kill must not delete jail artifacts or free the base claim behind the recovery state machine.
...
- Concurrent snapshot commands for one base serialize through the durable per-base intent; the loser fails before pause, artifact creation, or reservation.
- `rooms kill` refuses an exact pending snapshot transaction before signaling or reap, so it cannot bypass the GC fence or free the claim.
...
- Mocked failure tests cover output rejection under every managed room/jail tree and in both overlap directions for the snapshot/restore transaction indexes, exclusive per-base intent creation and a concurrent loser before pause, `rooms kill` refusal while that intent exists, writable jail-target staging/ownership/symlink refusal, pause, create, jail-to-host collection without losing Firecracker uid/gid readability, partial copies, missing/empty files, artifact sync, durable create-and-collect-success recording only after sync/validation, non-empty pairs without that marker, recoverable versus terminal partial-create classification, exact-claim abort, slot-token file/directory sync, GC before reservation, reservation-before-termination, concurrent GC at the handoff, failed egress/TAP teardown, incomplete jail/room teardown, metadata publication, indexed discovery with the base absent, and transaction recovery at every boundary.
