...
- Default `--out` to `<state-base>/snapshots/<snapshot-id>`, outside every managed cleanup tree. Canonicalize an explicit output and scan all room-state directories plus every default or custom jail-instance root; refuse the output at or below any tree that `RoomGuard` or GC may delete, not only the base room's trees.
...
- After Firecracker returns success, durably record the create-success transaction boundary before collection or reservation. Recovery may publish only a pair covered by that synced success marker; if the process/VMM died without it, treat even two non-empty files as an unrecoverable partial and take the terminal abort path rather than reserving or publishing.
- Before reservation or reap, explicitly collect the jail-created `/snapshot.vmstate` and `/snapshot.mem` into the plan's distinct host output paths. Copy each to an owner-only temporary file in `--out`, `sync_all`, rename into place, and sync the output directory; recovery resumes or validates this idempotent boundary before any jail deletion.
- Verify both collected host files exist and are non-empty before publishing metadata. Strengthen `snapshot::write_meta_atomic` to sync its temporary file and the parent directory around the rename. A completed `snapshot.json` must never advertise partial artifacts after a host crash.
...
- The default artifact directory survives cleanup; an output beneath any managed room or jail-instance cleanup tree is refused after canonicalization.
...
- Mocked failure tests cover output rejection under every managed room/jail tree, writable jail-target staging/ownership/symlink refusal, pause, create, durable create-success recording, non-empty partials without that marker, jail-to-host collection and partial copies, missing/empty files, artifact sync, recoverable versus terminal partial-create classification, exact-claim abort, slot-token file/directory sync, GC before reservation, reservation-before-termination, concurrent GC at the handoff, failed egress/TAP teardown, incomplete jail/room teardown, metadata publication, indexed discovery with the base absent, and transaction recovery at every boundary.
