...
- Canonicalize the snapshot artifact directory and any command `--out` before process creation or lease acquisition. Reject an output equal to, inside, or containing the current snapshot; the managed `<state-base>/snapshots`, `<state-base>/snapshot-intents`, or `<state-base>/restore-intents` trees; any artifact directory named by a live snapshot intent; or any valid completed snapshot directory found at an ancestor of or beneath the output. Collection must never delete or overwrite the current snapshot, a sibling/default snapshot, a custom-output snapshot, or snapshot/restore transaction recovery state.
...
- Restore refuses overlap with every known completed snapshot and with both snapshot/restore transaction-index trees before creating a process or lease; output cleanup cannot erase the current, sibling, custom-output, recovering snapshot, or any restore tombstone.
