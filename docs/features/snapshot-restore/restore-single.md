...
- Canonicalize the supplied `--image`, snapshot artifact directory, and any command `--out` before process creation or lease acquisition. Require output to be disjoint in both directions from the image path, the resolved rooms state base, and every default/custom jail-instance root, covering the backing rootfs, slots, room metadata, snapshots, transaction indexes, and live jail trees. Outside those managed roots, also reject an output equal to, inside, or containing the current snapshot; any artifact directory named by a live snapshot intent; or any valid completed custom snapshot directory found at an ancestor of or beneath the output. Collection must never delete the supplied image, rooms-managed state, or any current, sibling/default, custom-output, or recovering snapshot.
...
- Allocate the room id and planned room/jail paths, then durably create `<state-base>/restore-intents/<room-id>.json` before spawning any child. Its `PreSpawn` state records `snapshot_lineage`, the exact derived slot/tap, planned paths, egress policy, and no lease/TAP ownership; it survives room deletion and is sufficient for GC to classify a crash before launch.
- Start the launcher behind an inherited close-on-parent-death pipe barrier: it may not exec the jailer or Firecracker until the parent atomically adds the child PID/starttime to both the indexed intent and room-local metadata and syncs them. Only after both writes are durable does the parent release the barrier and allow an inert jailed process to start—still with no tap/network, snapshot load, or guest execution. A parent crash or breadcrumb-write failure closes the barrier, makes the child exit before exec, and acquires no lease. Leave `RoomMeta.slot` unset and TAP ownership false until lease success.
...
- Restore refuses output overlap with the canonical supplied image, entire resolved rooms state base, every jail-instance root, and every known custom completed/pending snapshot before creating a process or lease; cleanup cannot erase the backing rootfs, slots, room/jail state, snapshots, transaction indexes, or restore tombstones.
...
- A crash before the pre-spawn intent is durable starts no child and holds no lease. A crash after it but before barrier release leaves no executed jailer and is cleanable from the planned paths; a crash immediately before or after lease acquisition is classifiable as live or orphaned-dead. Cleanup consults the exact lease token before claiming TAP ownership, so a failed competing restore cannot delete the active restore's TAP.
...
Run `make check`. Unit tests assert lifecycle/output/witness parse admission; output rejection against the canonical image, resolved state base, and all jail roots in both ancestor and descendant directions, including slots, other rooms, snapshots, and both intent indexes; custom-snapshot overlap rejection; restore secret harvesting and nudge delivery; no SSH-env fallback; keep handoff; command teardown; ordering; read-only rootfs staging and hash refusal; bind-mount behavior; compatibility refusal; durable `PreSpawn` intent before child creation; barrier refusal to exec before PID/starttime persistence; child exit on parent/barrier failure; GC cleanup of a pre-spawn intent; non-owning room and indexed tombstone intent before lease; no lease on breadcrumb-write failure; durable lease/return token rewrites; a competing pre-lease cleanup never deleting the active TAP; idempotent GC immediately before and after leasing; checked egress/TAP teardown; lease return after clean reap; a crash between room deletion and lease return; lease retention after any incomplete resource cleanup; indexed reconstruction and GC-completed return; and the ACK gate. On the rooms-host: snapshot one sealed base, restore it twice with `--command --out` from the same unchanged image, exercise one `--keep` handoff, deliver one secret per restore, race a competing restore, inject crashes before spawn, before barrier release, and immediately before/after lease acquisition, force one compatibility mismatch and one incomplete-cleanup recovery, and prove distinct RNG, clock, key, identity, custody-before-resume, backing-image immutability, witness persistence, and reservation recovery.
