...
- Canonicalize the snapshot artifact directory and any command `--out` before process creation or lease acquisition. Reject an output equal to, inside, or containing the current snapshot; the managed `<state-base>/snapshots` or `<state-base>/snapshot-intents` trees; any artifact directory named by a live snapshot intent; or any valid completed snapshot directory found at an ancestor of or beneath the output. Collection must never delete or overwrite the current snapshot, a sibling/default snapshot, a custom-output snapshot, or transaction recovery state.
...
- Restore refuses overlap with every known completed or pending snapshot artifact/transaction tree before creating a process or lease; output cleanup cannot erase the current, sibling, custom-output, or recovering snapshot.
