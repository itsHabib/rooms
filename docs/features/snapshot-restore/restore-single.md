...
- Canonicalize the snapshot artifact directory and any command `--out` before process creation or lease acquisition. Require output to be disjoint in both directions from the resolved rooms state base and every default/custom jail-instance root, covering slots, room metadata, snapshots, transaction indexes, and live jail trees. Outside those managed roots, also reject an output equal to, inside, or containing the current snapshot; any artifact directory named by a live snapshot intent; or any valid completed custom snapshot directory found at an ancestor of or beneath the output. Collection must never delete rooms-managed state or any current, sibling/default, custom-output, or recovering snapshot.
...
- Restore refuses overlap with the entire resolved rooms state base, every jail-instance root, and every known custom completed/pending snapshot before creating a process or lease; output cleanup cannot erase slots, room/jail state, snapshots, transaction indexes, or restore tombstones.
