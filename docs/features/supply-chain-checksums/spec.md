...
**Cloud-written, host-gated merge.** The `src/doctor.rs` sha-drift check carries unit tests and passes `make check` in cloud CI; the host setup script + rootfs-builder changes (forced-mismatch bail, real downloads) can only be exercised on `rooms-host`, not a cloud VM. So the cursor cloud agent writes the code and proves the unit-testable slice, and the operator runs the forced-mismatch bail + doctor drift check on `rooms-host` before merge. Reconcile any drift between this spec and the current scripts before implementing (the builders moved to Alpine since the task was written). Batch 1 — parallel-safe with `harden-tap-rules` (disjoint files); `firecracker-under-jailer` rebases onto this task's `setup-rooms-host.sh` in batch 2.
